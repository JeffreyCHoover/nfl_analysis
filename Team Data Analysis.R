...
ungroup() %>%                                        # ungroup
  select(team, contains("z")) %>%                    # restrict vars to zscores
  group_by(team) %>%                                 # group by name and position
  summarise_at(vars(-team), funs(sumZ = sum(., na.rm = TRUE))) %>%    #sum zscores across years
